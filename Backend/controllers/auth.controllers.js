...
export const signup = async(req, res) => {
    try {
        let {firstname, lastname, username, email, password} = req.body;
        let existedUsername = await User.findOne({username});
        if (existedUsername) return res.status(400).json({message: "Username already exists!"});
        let existedEmail = await User.findOne({email});
        if (existedEmail) return res.status(400).json({message: "Email already exists!"});
        if (password.length < 8) return res.status(400).json({message: "Password must be at least 8-15 characters long"});
...
export const login = async(req, res) => {
    try {
        ...
        const isMatch = await bcrypt.compare(password, user.password);
        if (!isMatch) return res.status(400).json({message: "Your password is Invalid!"});
        let token = generateToken(user._id);
        const { password: _, ...safeUser } = user.toObject();
